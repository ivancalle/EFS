...
class ChiRSClassifier extends GeneralClassifier{
	...
	private def executeGenetic(): IndividualRS = {
		var pop = new Population(this.kBase, this.trainSet, 5000);

		var chromos = Array.fill(this.kBase.size())(true);
		var best = new IndividualRS(chromos);
		...
		do{	
			...
			best = pop.getFirsts(1)(0).asInstanceOf[IndividualRS];
	...
	private def generateRandomPopulation(n: Int): ArrayBuffer[IndividualRS] = {
		var individuals = ArrayBuffer[IndividualRS]();
		for( j <- 0 until n) {
			individuals += new IndividualRS(this.kBase.size());
		}
